...
export const getNextLocalTodoId = () => {
    ...
    const lastId = localTodos[localTodos.length - 1].id;
    const nextId = parseInt(lastId.split("-")[1]) + 1;  // split (local, 1) at index 1 convert to number  + 1
...
export const getLocalTodoById = (id) => {
    const todos = getLocalTodos();
