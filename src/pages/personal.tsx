...
function Personal() {
    ...
    const numOfPersonal = tasks?.data.filter(task => task.lists === 'personal')
    ...
    return (
        <main className="min-h-screen p-4 md:p-0 flex-1 flex flex-col justify-start items-start gap-4 md:w-full text-[#1a1a1a]">
            <div className="flex gap-4 items-center">
                ...
                <span className="py-1 p-3 border rounded font-semibold">{numOfPersonal?.length}</span>
            </div>

            {/* //list existing task for the current day  */}
            {numOfPersonal?.length === 0 ? (
                    <div className="mx-auto w-full">
                        <EmptyDemo setModalOpen={setIsDialogOpen} />
                    </div>
                ) : (<ul className="w-full flex flex-col gap-2">
                {numOfPersonal?.map((task) => (
                    <li key={task.id}>
                        <button
